...
import { currentSemesterProgress } from './utils/dailyPlan'

const API = import.meta.env.VITE_API_URL || ''

export default function AuthenticatedApp({ user, emailEnabled, setAuthState, page, setPage, dark, toggleDark, mobileMenuOpen, setMobileMenuOpen }) {
  ...
  const semesterProgress = useMemo(() => currentSemesterProgress(semester), [semester])
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-30 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center justify-between">
        <button aria-label={mobileMenuOpen ? "Close navigation" : "Open navigation"} aria-expanded={mobileMenuOpen} onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2 text-gray-600 dark:text-gray-400">
        ...
        <SyncStatus onReconnect={() => navigate('settings')} />
      </div>
      ...
      <div className={`fixed inset-y-0 left-0 z-20 transform transition-transform duration-200 lg:relative lg:transform-none ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <Sidebar active={page} onNavigate={navigate} dark={dark} toggleDark={toggleDark} urgentCount={urgentCount} streak={streak} semesterProgress={semesterProgress} semesterName={semesterProgress !== null ? semester?.name : 'Your workspace'} user={user} onLogout={handleLogout} />
      </div>
